...
export function heliportInstalledReadme(id: HeliportIdentity, projectName: string): string {
  return [
    ...
    "",
    `Then open LOCATION and find "${sanitizeValue(id.name).trim()}" on the map, where you built it.`,
    "Aerofly's search box may not list a new airport by its code: a heliport that loads and flies",
    "perfectly can still return nothing when you type its code there. The map is the reliable route.",
    "",
